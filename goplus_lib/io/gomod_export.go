...
func execNopCloser(_ int, p *gop.Context) {
	args := p.GetArgs(1)
	ret0 := io.NopCloser(toType1(args[0]))
	p.Ret(1, ret0)
}
...
func execReadAll(_ int, p *gop.Context) {
	args := p.GetArgs(1)
	ret0, ret1 := io.ReadAll(toType1(args[0]))
	p.Ret(1, ret0, ret1)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("NopCloser", io.NopCloser, execNopCloser),
		...
		I.Func("ReadAll", io.ReadAll, execReadAll),
	...
	I.RegisterTypes(
		...
		I.Type("ReadSeekCloser", reflect.TypeOf((*io.ReadSeekCloser)(nil)).Elem()),
