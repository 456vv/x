...
func execmMapLoadAndDelete(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	ret0, ret1 := args[0].(*sync.Map).LoadAndDelete(args[1])
	p.Ret(2, ret0, ret1)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("(*Map).LoadAndDelete", (*sync.Map).LoadAndDelete, execmMapLoadAndDelete),
