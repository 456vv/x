...
func execmFileHeaderSetMode(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	args[0].(*zip.FileHeader).SetMode(args[1].(fs.FileMode))
	p.PopN(2)
}

func toType0(v interface{}) fs.FileInfo {
	if v == nil {
		return nil
	}
...
func execmReaderOpen(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	ret0, ret1 := args[0].(*zip.Reader).Open(args[1].(string))
	p.Ret(2, ret0, ret1)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("(*Reader).Open", (*zip.Reader).Open, execmReaderOpen),
