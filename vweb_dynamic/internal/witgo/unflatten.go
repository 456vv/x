...
// ============================================================
// Unflatten：uint64 参数流 -> Go 值
// ============================================================

// unflattenParam 从参数流 ps 中读取值，还原为 targetType 类型的 Go 值。
// 与 flattenParam 相反的操作，从扁平化的 uint64 流中恢复原始 Go 值。
//
// 参数:
//   - ctx:       上下文
//   - mem:       Guest 内存（用于读取字符串/切片内容）
//   - ps:        参数流（顺序消费）
//   - targetType: 目标 Go 类型
//
// 返回:
//   - reflect.Value: 还原的 Go 值
//   - error: 读取失败时的错误
//
// 示例:
//
//	val, err := h.unflattenParam(ctx, mem, ps, reflect.TypeOf(42))
