...
func execmURLEscapedFragment(_ int, p *gop.Context) {
	args := p.GetArgs(1)
	ret0 := args[0].(*url.URL).EscapedFragment()
	p.Ret(1, ret0)
}
...
func execmURLRedacted(_ int, p *gop.Context) {
	args := p.GetArgs(1)
	ret0 := args[0].(*url.URL).Redacted()
	p.Ret(1, ret0)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("(*URL).EscapedFragment", (*url.URL).EscapedFragment, execmURLEscapedFragment),
		I.Func("(*URL).String", (*url.URL).String, execmURLString),
		I.Func("(*URL).Redacted", (*url.URL).Redacted, execmURLRedacted),
