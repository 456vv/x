	...
	req.BodyHandle, req.Body, req.BodyWriter = i.hm.NewOutgoingBody(contentLength, func(trailers manager_http.Fields) error {
		if req.Request != nil {
			if req.Request.Trailer == nil {
				req.Request.Trailer = make(manager_http.Fields)
			}
			// maps.Copy 需要 Go 1.21；手写拷贝以兼容更旧编译器
			for k, vv := range trailers {
				req.Request.Trailer[k] = append(req.Request.Trailer[k], vv...)
			}
		}
		return nil
	})
...
func (i *outgoingRequestImpl) Headers(_ context.Context, this OutgoingRequest) Headers {
	...
	req.EnsureHeadersHandle(func() {
		...
		req.HeadersHandle = i.hm.Fields.Add(manager_http.Fields(req.Headers))
	})
	return req.HeadersHandle
}
