...
func execFormatComplex(_ int, p *gop.Context) {
	args := p.GetArgs(4)
	ret0 := strconv.FormatComplex(args[0].(complex128), args[1].(byte), args[2].(int), args[3].(int))
	p.Ret(4, ret0)
}
...
func execParseComplex(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	ret0, ret1 := strconv.ParseComplex(args[0].(string), args[1].(int))
	p.Ret(2, ret0, ret1)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("FormatComplex", strconv.FormatComplex, execFormatComplex),
		...
		I.Func("ParseComplex", strconv.ParseComplex, execParseComplex),
