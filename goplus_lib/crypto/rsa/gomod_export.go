...
func execmPrivateKeyEqual(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	ret0 := args[0].(*rsa.PrivateKey).Equal(args[1])
	p.Ret(2, ret0)
}
...
func execmPublicKeyEqual(_ int, p *gop.Context) {
	args := p.GetArgs(2)
	ret0 := args[0].(*rsa.PublicKey).Equal(args[1])
	p.Ret(2, ret0)
}
...
func init() {
	I.RegisterFuncs(
		...
		I.Func("(*PrivateKey).Equal", (*rsa.PrivateKey).Equal, execmPrivateKeyEqual),
		...
		I.Func("(*PublicKey).Equal", (*rsa.PublicKey).Equal, execmPublicKeyEqual),
